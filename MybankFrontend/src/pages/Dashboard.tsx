...
const QuickAction: React.FC<{ to: string; icon: React.ReactNode; label: string }> = ({ to, icon, label }) => (
    <ReactRouterDOM.Link to={to} className="flex flex-col items-center justify-center p-4 bg-white dark:bg-slate-800 rounded-xl shadow-md hover:shadow-lg hover:-translate-y-1 transition-all duration-300 space-y-2">
...
const AccountCard: React.FC<{ account: Account }> = ({ account }) => (
    <Card className="flex flex-col justify-between">
      <div>
        ...
        <p className={`text-2xl sm:text-3xl font-bold mt-4 ${account.balance >= 0 ? 'text-gray-800 dark:text-white' : 'text-danger'}`}>
...
const Dashboard: React.FC = () => {
    ...
    return (
        <div className="space-y-6">
            ...
            <div>
                <h2 className="text-xl sm:text-2xl font-bold mb-4">My Accounts</h2>
